...
void DualSceneDrawer::Draw(ID3D12GraphicsCommandList* cmdList,D3D12_GPU_DESCRIPTOR_HANDLE prevSceneHandle, D3D12_GPU_DESCRIPTOR_HANDLE nextSceneHandle, D3D12_GPU_DESCRIPTOR_HANDLE weightHandle) {
	...
	*wvpData_ = wvpMatrix;*/
	...
	cmdList->SetGraphicsRootDescriptorTable(3, weightHandle);
...
void DualSceneDrawer::TransferVertex() {
...
void DualSceneDrawer::CreateGraphicsPipelineState() {
	...
	rootParameters[3].DescriptorTable.pDescriptorRanges = descriptorRangeWeight; //Tableの中身の配列を指定
	rootParameters[3].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeWeight); //Tableで利用する数
