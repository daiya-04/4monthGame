...
void GPUParticle::StaticInit() {
	...
	initParticleRootParams[(size_t)InitParticleRootParam::kParticleNum].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	initParticleRootParams[(size_t)InitParticleRootParam::kParticleNum].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
	initParticleRootParams[(size_t)InitParticleRootParam::kParticleNum].Descriptor.ShaderRegister = 0;
	...
	emitterRootParams[(size_t)EmitterRootParam::kParticleNum].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	emitterRootParams[(size_t)EmitterRootParam::kParticleNum].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
	emitterRootParams[(size_t)EmitterRootParam::kParticleNum].Descriptor.ShaderRegister = 2;
	...
	updateRootParams[(size_t)UpdateRootParam::kParticleNum].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	updateRootParams[(size_t)UpdateRootParam::kParticleNum].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
	updateRootParams[(size_t)UpdateRootParam::kParticleNum].Descriptor.ShaderRegister = 1;
...
void GPUParticle::Init(uint32_t textureHandle, int32_t particleNum) {
	...
	commandList_->SetComputeRootConstantBufferView((UINT)InitParticleRootParam::kParticleNum, particleNumBuff_->GetGPUVirtualAddress());
...
void GPUParticle::Update() {
	...
	*emitterData_ = emitter_;
	...
	commandList_->SetComputeRootConstantBufferView((UINT)EmitterRootParam::kParticleNum, particleNumBuff_->GetGPUVirtualAddress());
	...
	commandList_->SetComputeRootConstantBufferView((UINT)UpdateRootParam::kParticleNum, particleNumBuff_->GetGPUVirtualAddress());
...
void GPUParticle::Draw(const Camera& camera) {
	...
	perViewData_->viewProjectionMat_ = camera.GetMatView() * camera.GetMatProjection();
...
void GPUParticle::CreateBuffer() {
	...
	//左下
	vertices[0].pos_ = { -0.5f,-0.5f,0.0f,1.0f };
	vertices[0].uv_ = { 0.0f,1.0f };
	//左上
	vertices[1].pos_ = { -0.5f,0.5f,0.0f,1.0f };
	vertices[1].uv_ = { 0.0f,0.0f };
	//右下
	vertices[2].pos_ = { 0.5f,-0.5f,0.0f,1.0f };
	vertices[2].uv_ = { 1.0f,1.0f };

	//右上
	vertices[3].pos_ = { 0.5f,0.5f,0.0f,1.0f };
	...
	emitterBuff_ = CreateBufferResource(device_, sizeof(EmitterSquare));
	...
	particleNumBuff_ = CreateBufferResource(device_, sizeof(ParticleNum));

	particleNumBuff_->Map(0, nullptr, reinterpret_cast<void**>(&particleNumData_));
	particleNumData_->num = maxParticleNum_;
